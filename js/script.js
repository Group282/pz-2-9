...
document.getElementById(`kit_img_search`).addEventListener('dragstart', (ev) => {
...
function startGame() {
  ...
  $("#kit_img_search").attr("src", `img/${search_arr[0]}.jpg`);
  let counter = 0
  let jq_rows = $('.kit_game_table').children().children()
  for (let row of jq_rows) {
    let jq_cells = row.children
    for (let cell of jq_cells) {
      $(cell).html(`<img id="${game_arr[counter]}_id_img" class="kit_img" src="img/${game_arr[counter++]}.jpg"/>`);
    }
  }
  ...
  for (let j of game_arr) {
    ...
    document.getElementById(`${j}_id_img`).addEventListener("drop", (ev) => {
      ...
      if (data === j.toString()) {
        $("#kit_img_search").attr("src", `img/${search_arr[++search_counter]}.jpg`);
        $(`#${j}_id_img`).remove()
